...
class QueryExecutorHelper implements QueryExecutorInterface
{
    ...
    public function executeQuery(
        ...
        if (false === $result) {
            throw new \RuntimeException(sprintf('Query failed. Error: %s. Statement: %s', $pdo->errorInfo()[2] ?? 'Unknown error', $query));
        }
    ...
    {
        if (!is_object($arg3)) {
            throw new \InvalidArgumentException('When using FETCH_INTO, the third argument must be an object.');
        }
