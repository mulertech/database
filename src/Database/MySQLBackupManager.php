...
class MySQLBackupManager
{
    ...
    {
        $output = [];
        $tables = null !== $tableList ? implode(' ', $tableList).' ' : '';
    ...
    private function checkPasswordQuotes(): void
    {
        ...
        if (str_contains($passwordStr, "'") && str_contains($passwordStr, '"')) {
            throw new \RuntimeException('The password have both single and double quote, please change the password to use only one type of quote.');
        }
    }
